...
class AlphaBetaSearch(BaseSearch):
    ...
    def search(self, board, depth: int) -> SearchResult:
        ...
        for move in self.move_generator.generate(board):
            ...
            score = self._alpha_beta(board, depth - 1, alpha, beta, maximizing_player=False)
        ...
        return SearchResult(best_move, int(best_score), depth, self.nodes_visited, elapsed, "Alpha-Beta")

    def _alpha_beta(self, board, depth: int, alpha: float, beta: float, maximizing_player: bool) -> int:
