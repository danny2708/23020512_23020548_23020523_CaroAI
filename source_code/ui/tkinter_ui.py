...
class AIVsAIFrame(ttk.Frame):
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Button(controls, text="New game", command=self.new_game).grid(row=1, column=4, pady=(8, 0))
        ttk.Button(controls, text="Step", command=self.step_once).grid(row=1, column=5, pady=(8, 0))
        self.run_button = ttk.Button(controls, text="Run all", command=self.toggle_run)
        self.run_button.grid(row=1, column=6, padx=(8, 0), pady=(8, 0))

        body = ttk.Frame(self)
        body.pack(fill=tk.BOTH, expand=True)

        self.board_view = BoardView(body)
        self.board_view.pack(side=tk.LEFT, anchor=tk.N)

        side_panel = ttk.Frame(body, padding=(14, 0, 0, 0))
        side_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        ttk.Label(side_panel, textvariable=self.status_var, wraplength=390).pack(anchor=tk.W)
        self.log = tk.Text(side_panel, width=62, height=22, state=tk.DISABLED)
        self.log.pack(fill=tk.BOTH, expand=True, pady=(10, 0))
    ...
    def new_game(self) -> None:
        ...
        self.board_view.build(size)
        self.board_view.refresh_board(self.game.board)
        self.status_var.set("Ready. Use Step or Run all.")
        self._clear_log()
        self._append_log(f"New AI vs AI game. X={x_mode}, O={o_mode}, depth={depth}.")

    def step_once(self) -> None:
        if self.game is None or self.busy or self.running:
            return
        if not self._ensure_current_settings():
            return
        if self.game.status() != "ONGOING":
            ...
            return
        self._start_step(keep_running=False)
    ...
    def toggle_run(self) -> None:
        ...
        if self.running:
            self.running = False
            self.run_button.configure(text="Run all")
            self.status_var.set("Auto play paused.")
            return
        ...
        self._start_step(keep_running=True)

    def _start_step(self, keep_running: bool) -> None:
        if self.game is None or self.busy:
            return
        ...
        threading.Thread(target=self._run_one_step, args=(keep_running,), daemon=True).start()

    def _run_one_step(self, keep_running: bool) -> None:
        assert self.game is not None
        step = self.game.step()
        grid = [row[:] for row in self.game.board.grid]
        status = self.game.status()
        self.after(0, self._finish_step, step, grid, status, keep_running)
    ...
    def _finish_step(
        ...
        if status == "ONGOING":
            if keep_running and self.running:
                ...
                self.after(80, lambda: self._start_step(keep_running=True))
            else:
                self.status_var.set("Ready for next step.")
        else:
            self.status_var.set(f"Game over: {status}")
            self._append_log(f"Game over: {status}")
            self.running = False
            self.run_button.configure(text="Run all")
